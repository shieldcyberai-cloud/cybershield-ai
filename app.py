...
<style>
    ...
    div[data-testid="stHorizontalBlock"]:has(input[placeholder="Ask anything"]) [data-testid="stFileUploader"] button:hover::after {{
        color: #f8fafc !important;
    }}

    /* --- RED BORDER SUPPRESSION FOR TEXT INPUT --- */
    ...
    div[data-testid="stHorizontalBlock"]:has(input[placeholder="Ask anything"]) .stTextInput > div > div > input:active,
    div[data-testid="stHorizontalBlock"]:has(input[placeholder="Ask anything"]) .stTextInput > div > div > input:focus-visible {{
        border: none !important;
        outline: none !important;
        box-shadow: none !important;
    }}
    div[data-testid="stHorizontalBlock"]:has(input[placeholder="Ask anything"]) .stTextInput > div > div > input::placeholder {{
        color: #a1a1aa !important;
        opacity: 1 !important;
    }}

    /* --- ılı VOICE TRIGGER COMPONENT --- */
    ...
    div[data-testid="stHorizontalBlock"]:has(input[placeholder="Ask anything"]) button[kind="primary"]:hover {{
...
def clean_text_for_speech(text):
    """Sanitizes Markdown or structural tokens from text prior to TTS synthesis."""
    if not text: return ""
    text = re.sub(r'[*_#`~>\[\]]', '', text) 
    return text.strip()

def speak(text, priority="Normal"):
    """Translates text data into synthesized speech streams via Edge-TTS."""
    if not text: return
    try:
            ...
            if st.session_state['auth_mode'] == "Forgot Password":
                ...
                if st.session_state['reset_step'] == 1:
                    ...
                    if st.button("Send Recovery OTP", type="primary"):
                        if e_reset:
                            fetched_username = get_username_by_email(e_reset)
                            
                            if fetched_username:
                                otp = str(random.randint(100000, 999999))
                                with st.spinner("Transmitting Authorization Code via secure channel..."):
                                    success, status_msg = send_otp_email(e_reset, otp, fetched_username)
                                    
                                    if success:
                                        st.session_state['reset_otp'] = otp
                                        st.session_state['reset_email'] = e_reset
                    ...
                    if st.button("Reset Password", type="primary"):
                        if otp_inp == st.session_state['reset_otp']:
                            if new_pass:
                                if update_user_password(st.session_state['reset_email'], new_pass):
...
Input suspicious text payloads manually into the text area, or utilize the <strong>Optical Character Recognition (OCR)</strong> module to extract textual data directly from uploaded image evidence (e.g., screenshots of SMS, emails, or websites). Click <strong>"Analyze Threat Level"</strong> to initialize the heuristic Machine Learning engine and generate a statistical threat probability score.
</p>
</div>
<div style="margin-bottom: 15px;">
<h4 style="color: #e2e8f0; font-size: 1rem; margin-bottom: 5px;">2. AI Escalation Protocol</h4>
<p style="color: #94a3b8; font-size: 0.9rem; line-height: 1.6; margin-top: 0;">
If a payload is flagged as suspicious or requires further context, utilize the <strong>"Send to AI Assistant"</strong> directive. This will forward the raw payload and its preliminary threat score directly to the Cyber Operative for deep contextual analysis and remediation recommendations.
</p>
</div>
<div>
<h4 style="color: #f87171; font-size: 1rem; margin-bottom: 5px;">3. Security & Compliance Notice</h4>
<p style="color: #94a3b8; font-size: 0.9rem; line-height: 1.6; margin-top: 0;">
While all telemetry is processed securely, operators are advised <strong>not</strong> to input highly sensitive Personal Identifiable Information (PII) or unmasked enterprise credentials into the scanner. The system is calibrated strictly for threat vector identification.
</p>
</div>
</div>
        """, unsafe_allow_html=True)
    ...
    # ==================================================
    # 🤖 CYBER ASSISTANT MODULE (WITH CUSTOM INLINE UI)
    # ==================================================
        ...
        with c_input:
            # Note: Browser autocomplete is intentionally disabled
        ...
        if st.session_state.get('auto_send_msg'): 
            ...
            if chat_img is not None:
                # Execute when an image is present, with or without accompanying text
            ...
            # Purge the temporary submission state
